  ...
  return (
    <div className="p-4 md:p-8 max-w-[1400px]">
        ...
        <div className="flex flex-wrap items-start gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-4 pt-4 border-t border-border">
              <div><p className="text-xs text-muted-foreground">Wingspan</p><p className="text-sm font-bold text-foreground">{player.wingspan}</p></div>
              <div><p className="text-xs text-muted-foreground">Standing Reach</p><p className="text-sm font-bold text-foreground">{player.standingReach}</p></div>
              <div><p className="text-xs text-muted-foreground">Vertical Leap</p><p className="text-sm font-bold text-foreground">{player.verticalLeap}&quot;</p></div>
              <div><p className="text-xs text-muted-foreground">Stride Length</p><p className="text-sm font-bold text-foreground">{player.strideLength}m</p></div>
              <div><p className="text-xs text-muted-foreground">Weight</p><p className="text-sm font-bold text-foreground">{player.weight} lbs</p></div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex-1 space-y-3">
                ...
                <div className="flex gap-4 mt-3">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground"><span className="w-3 h-3 rounded-full bg-green-500" /> Made</div>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground"><span className="w-3 h-3 rounded-full bg-red-500" /> Missed</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
                ...
                <tbody>
                  {playerMeals.map((m) => (
                    <tr key={m.id} className="border-b border-border last:border-0">
                      <td className="px-4 py-3"><span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 capitalize">{m.meal}</span></td>
          ...
          <div className="bg-card border border-border rounded-xl p-6">
            ...
            <div className="space-y-3">
              <ProgressBar label="Protein" value={playerMeals.reduce((a, m) => a + m.protein, 0)} max={250} color="bg-blue-500" showPercent={false} />
              <ProgressBar label="Carbohydrates" value={playerMeals.reduce((a, m) => a + m.carbs, 0)} max={400} color="bg-yellow-500" showPercent={false} />
              <ProgressBar label="Fat" value={playerMeals.reduce((a, m) => a + m.fat, 0)} max={120} color="bg-red-400" showPercent={false} />
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'recovery' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <StatCard label="Latest Recovery" value={playerRest[0]?.recoveryScore || 'N/A'} subValue={playerRest[0]?.date} size="sm" />
            <StatCard label="Avg Sleep" value={playerRest.length > 0 ? `${(playerRest.reduce((a, r) => a + r.sleepHours, 0) / playerRest.length).toFixed(1)}h` : 'N/A'} size="sm" />
            <StatCard label="Avg Sleep Quality" value={playerRest.length > 0 ? `${(playerRest.reduce((a, r) => a + r.sleepQuality, 0) / playerRest.length).toFixed(1)}/5` : 'N/A'} size="sm" />
            <StatCard label="Soreness Status" value={playerRest[0]?.soreness || 'N/A'} size="sm" />
          </div>
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {playerRest.map((r) => (
                    <tr key={r.id} className="border-b border-border last:border-0">
                      ...
                      <td className="px-3 py-3 text-center">
                        <span className={`text-sm font-bold ${r.recoveryScore >= 85 ? 'text-green-500' : r.recoveryScore >= 70 ? 'text-yellow-500' : 'text-red-500'}`}>
                          {r.recoveryScore}
                        </span>
                      </td>
                      <td className="px-3 py-3 text-center">
            ...
            <div className="flex items-end gap-2 h-32">
              {playerRest.map((r) => (
                <div key={r.id} className="flex-1 flex flex-col items-center gap-1">
                  <div
                    className={`w-full rounded-t ${r.recoveryScore >= 85 ? 'bg-green-500' : r.recoveryScore >= 70 ? 'bg-yellow-500' : 'bg-red-500'}`}
